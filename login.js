...
export default class loginEvent extends EventListener {
  ...
  async getTicket (url) {
    ...
    let ticket = ''
    let ws = new WebSocket(req)
    ws.on('error', () => {})
    ws.on('open', () => {
      ws.send(JSON.stringify({ type: 'register', payload: { url } }))
      console.log('\n----请打开下方链接并在2分钟内进行验证----')
    ...
    ws.on('message', async (msg) => {
      try {
        ...
        if (data.type == 'ticket') {
          ...
          let { url, ...opts } = data.payload
          /** 仅使用IPv4 */
          opts.agent = new Agent({ family: 4 })
          let rsp = await fetch(url, opts)
          let arrBuf = await rsp.arrayBuffer()
          data.payload = {
